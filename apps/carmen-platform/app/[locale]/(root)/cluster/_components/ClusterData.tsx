...
export default function ClusterData({ clusterData, view }: Props) {
    ...
    <div>
            ...
            {clusterData?.map((cluster: GetClusterDto, index: number) => (
              <TableRow key={cluster.id}>
                ...
                <TableCell className="font-semibold">
                  <Link href={`/cluster/${cluster.id}`} className="hover:underline">
                    {cluster.name}
                  </Link>
                </TableCell>
      ...
      {(!clusterData || clusterData.length === 0) && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-muted-foreground">There are no clusters to display at the moment.</p>
        </div>
      )}
    </div>
  );
