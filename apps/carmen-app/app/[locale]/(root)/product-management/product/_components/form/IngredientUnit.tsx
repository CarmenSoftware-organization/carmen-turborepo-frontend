...
const IngredientUnit = ({ control, currentMode }: IngredientUnitProps) => {
  ...
  const handleAddUnit = useCallback(
    ...
    [prependIngredientUnit, inventoryUnitId]
  );
    ...
    (index: number, isDataField: boolean, checked: boolean) => {
      ...
      if (unitsData?.data) {
        unitsData.data.forEach((unit, i: number) => {
          ...
          setValue(
            `ingredient_units.data.${i}.is_default` as `ingredient_units.data.${number}.is_default`,
            false,
            { shouldDirty: true }
          );
      ...
      if (unitsData?.add) {
        unitsData.add.forEach((_, i: number) => {
          ...
          setValue(
            `ingredient_units.add.${i}.is_default` as `ingredient_units.add.${number}.is_default`,
            false,
            { shouldDirty: true }
          );
        });
      ...
      if (isDataField && unitsData?.data) {
        setValue(
          `ingredient_units.data.${index}.is_default` as `ingredient_units.data.${number}.is_default`,
          true,
          { shouldDirty: true }
        );
        ...
        setValue(
          `ingredient_units.add.${index}.is_default` as `ingredient_units.add.${number}.is_default`,
          true,
          { shouldDirty: true }
        );
      }
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("inventory_unit")} />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        accessorKey: "to_unit",
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("ingredient_unit")} />
        ),
        cell: ({ row }) => {
          const unit = row.original;
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("default")} />
        ),
        cell: ({ row }) => {
          const unit = row.original;
            ...
            return (
              <div className="flex justify-center">
                <FormField
                  control={control}
                  name={
                    `ingredient_units.add.${unit.fieldIndex!}.is_default` as `ingredient_units.add.${number}.is_default`
                  }
                  render={({ field }) => (
                    <FormItem className="space-y-0">
                      <FormControl>
            ...
            return (
              <div className="flex justify-center">
                <FormField
                  control={control}
                  name={
                    `ingredient_units.data.${unit.dataIndex}.is_default` as `ingredient_units.data.${number}.is_default`
                  }
                  render={({ field }) => (
                    <FormItem className="space-y-0">
                      <FormControl>
      ...
      },
      {
        id: "conversion",
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("conversion")} />
        ),
        cell: ({ row }) => {
          const unit = row.original;
          return (
            <ConversionPreviewWatcher control={control} unit={unit} getUnitName={getUnitName} />
          );
        },
        enableSorting: false,
        size: 180,
                      ...
                      <AlertDialogContent className="max-w-md">
                        <AlertDialogHeader>
                          <AlertDialogTitle className="text-xl">
                            {tCommon("delete")} {tProducts("ingredient_unit")}
                          </AlertDialogTitle>
                          <AlertDialogDescription>
