    ...
    (index: number, isDataField: boolean, checked: boolean) => {
      ...
      if (unitsData?.data) {
        unitsData.data.forEach((unit, i: number) => {
          ...
          setValue(
            `order_units.data.${i}.is_default` as `order_units.data.${number}.is_default`,
            false,
            { shouldDirty: true }
          );
      ...
      if (unitsData?.add) {
        unitsData.add.forEach((_, i: number) => {
          ...
          setValue(
            `order_units.add.${i}.is_default` as `order_units.add.${number}.is_default`,
            false,
            { shouldDirty: true }
          );
        });
      ...
      if (isDataField && unitsData?.data) {
        setValue(
          `order_units.data.${index}.is_default` as `order_units.data.${number}.is_default`,
          true,
          { shouldDirty: true }
        );
        ...
        setValue(
          `order_units.add.${index}.is_default` as `order_units.add.${number}.is_default`,
          true,
          { shouldDirty: true }
        );
      }
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("order_unit")} />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        accessorKey: "to_unit",
        header: ({ column }) => (
          <DataGridColumnHeader column={column} title={tProducts("to_inventory_unit")} />
        ),
        cell: ({ row }) => {
          const unit = row.original;
            ...
            return (
              <div className="flex justify-center">
                <FormField
                  control={control}
                  name={
                    `order_units.add.${unit.fieldIndex!}.is_default` as `order_units.add.${number}.is_default`
                  }
                  render={({ field }) => (
                    <FormItem className="space-y-0">
                      <FormControl>
            ...
            return (
              <div className="flex justify-center">
                <FormField
                  control={control}
                  name={
                    `order_units.data.${unit.dataIndex}.is_default` as `order_units.data.${number}.is_default`
                  }
                  render={({ field }) => (
                    <FormItem className="space-y-0">
                      <FormControl>
                      ...
                      <AlertDialogContent className="max-w-md">
                        <AlertDialogHeader>
                          <AlertDialogTitle className="text-xl">
                            {tCommon("delete")} {tProducts("order_unit")}
                          </AlertDialogTitle>
                          <AlertDialogDescription>
