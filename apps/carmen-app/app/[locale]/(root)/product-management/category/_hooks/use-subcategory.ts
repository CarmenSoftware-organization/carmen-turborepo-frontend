...
export const useSubCategoryQuery = ({
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const result = await getAllApiRequest(API_URL, token, "Error fetching subcategory", params);
        return result;
      } catch (error) {
...
export const useSubCategoryMutation = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  const API_URL = subCategoryApiUrl(buCode);
  return useMutation({
    mutationFn: async (data: SubCategoryDto) => {
      if (!token || !buCode) throw new Error("Unauthorized");
      return await postApiRequest(API_URL, token, data, "Error creating subcategory");
    },
    onSuccess: () => {
...
export const useUpdateSubCategory = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: SubCategoryDto) => {
...
export const useDeleteSubCategory = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  return useMutation({
