...
import { mockStockData } from "@/mock-data/store-operation";
import { StockDataItemDto } from "@/dtos/store-operation.dto";
export default function StockTable() {
  const [items, setItems] = useState<StockDataItemDto[]>(mockStockData);
  return (
    <Card>
      <CardHeader>
            ...
            <TableBody>
              {items.map((item) => {
                const suggestedOrder = Math.max(
                  0,
                  item.parLevel - (item.currentStock + item.onOrder)
                );
                ...
                return (
                  <TableRow key={item.id}>
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          setItems(
                            items.map((i) =>
                              i.id === item.id ? { ...i, selected: checked === true } : i
                            )
                          );
                        }}
                      />
                    ...
                    <TableCell className="text-right">
                      <span
                        className={`${
                          item.currentStock < item.reorderPoint ? "text-red-500 font-medium" : ""
                        }`}
                      >
                        {item.currentStock}
                      </span>
                    ...
                    <TableCell className="text-right">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setItems(
                            items.map((i) => (i.id === item.id ? { ...i, orderAmount: value } : i))
                          );
                        }}
                      />
                    </TableCell>
                    <TableCell>
                      <Select
                        ...
                        onValueChange={(value) => {
                          setItems(
                            items.map((i) => (i.id === item.id ? { ...i, unit: value } : i))
                          );
                        }}
                      >
