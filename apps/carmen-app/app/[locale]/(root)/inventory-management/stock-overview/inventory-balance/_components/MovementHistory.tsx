import { InventoryBalanceItemDto, mockMovementHistoryList } from "@/mock-data/inventory-balance";
import {
  Table,
  TableBody,
  TableCell,
...
export default function MovementHistory() {
  ...
  const getSortIcon = (field: keyof InventoryBalanceItemDto) => {
    if (sortField !== field)
      return <ArrowRight className="h-4 w-4 text-muted-foreground opacity-50" />;
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-[80px] cursor-pointer" onClick={() => handleSort("time")}>
              <div className="flex items-center gap-1">
                Time
                {getSortIcon("time")}
              </div>
