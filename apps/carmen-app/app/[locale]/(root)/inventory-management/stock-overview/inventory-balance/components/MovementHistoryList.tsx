        ...
        <Table>
            ...
            <TableBody>
                {mockBalanceReport.map((section) => {
                    ...
                    const totalQuantity = section.sub_item?.reduce(
                        (sum, subItem) => sum + subItem.total_item_quantity, 0
                    ) ?? 0;
                    const totalValue = section.sub_item?.reduce(
                        (sum, subItem) => sum + subItem.items.reduce(
                            (itemSum, item) => itemSum + item.value, 0
                        ), 0
                    ) ?? 0;
                    ...
                    return (
                        <>
                            ...
                            {isExpanded && section.sub_item?.map((subItem) => {
                                const isSubExpanded = expandedSubSections[`${section.id}-${subItem.name}`] || false;
                                ...
                                return (
                                    <>
                                        ...
                                        >
                                            ...
                                            <TableCell className="text-right">{formatNumber(subItem.total_item_quantity)}</TableCell>
                                            <TableCell className="text-right">${formatNumber(subItemTotalValue)}</TableCell>
                                        </TableRow>

                                        {isSubExpanded && subItem.items.map((item) => (
                                            <TableRow
                                                key={item.id}
                                            ...
                                            >
                                                ...
                                                <TableCell className="text-right">{item.unit_cost} ({item.unit})</TableCell>
