...
import { grnPostSchema } from "@/app/[locale]/(root)/procurement/goods-received-note/_schemas/grn-form.schema";
...
export default function FormGrn({ mode, initialValues }: FormGrnProps) {
  ...
  const { isPending: isCreatePending } = useGrnMutation(token || "", buCode || "");
  ...
  const defaultValues: CreateGRNDto = {
    ...
    current_workflow_status: initialValues?.workflow_current_stage ?? "pending",
    ...
    payment_due_date: initialValues?.payment_due_date ?? new Date().toISOString(),
