...
export default function UserDashboard() {
  ...
  const connectWebSocket = () => {
    ...
    ws.onopen = () => {
      ...
      addMessage({
        type: "connection",
        data: { message: `Connected as ${user_id}` },
  ...
  const createSystemNotification = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addMessage({
          type: "api",
          data: { message: "System notification created (should appear in real-time)" },
          timestamp: new Date().toISOString(),
        });
      }
    } catch {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addMessage({
          type: "api",
          data: {
            message: `User notification sent to ${recipient_id} (check their dashboard for real-time update)`,
          },
          timestamp: new Date().toISOString(),
        });
      }
  ...
  const toggleCronJob = async (jobId: string, action: "start" | "stop") => {
    try {
      ...
      if (response.ok) {
        addMessage({
          type: "api",
          data: { message: `CronJob ${action}ed (status update should appear in real-time)` },
          timestamp: new Date().toISOString(),
        });
        fetchCronJobs();
      }
      ...
      addMessage({
        type: "error",
        data: { message: `Failed to ${action} cronjob` },
        timestamp: new Date().toISOString(),
      });
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {user.platform_role === "admin" && (
            <div className="p-4 border rounded-lg">
              <h3 className="text-lg font-semibold mb-4">
                📢 System Notification (Broadcast to All)
              </h3>
                ...
                {cronJobs.map((job) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-1 text-xs rounded ${job.isActive ? "border" : "border opacity-50"}`}
                      >
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 border rounded-lg">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {notifications.map((notif, index) => (
                <div
                  key={index}
                  className={`p-3 rounded border-l-4 ${getNotificationStyles(notif.type)}`}
                >
          ...
          <div className="p-4 border rounded-lg">
            <h3 className="text-lg font-semibold mb-4">
              📝 WebSocket Activity Log ({messages.length})
            </h3>
