...
export const useItemGroupMutation = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  const API_URL = itemGroupApiUrl(buCode);
  return useMutation({
    mutationFn: async (data: ItemGroupDto) => {
      if (!token || !buCode) throw new Error("Unauthorized");
      return await postApiRequest(API_URL, token, data, "Error creating item group");
    },
    onSuccess: () => {
...
export const useUpdateItemGroup = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: ItemGroupDto) => {
      if (!token || !buCode || !data.id) throw new Error("Unauthorized");
      const API_URL_BY_ID = itemGroupApiUrl(buCode, data.id);
      return await updateApiRequest(API_URL_BY_ID, token, data, "Error updating item group", "PUT");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["itemgroup", buCode] });
    },
...
export const useDeleteItemGroup = (token: string, buCode: string) => {
  const queryClient = useQueryClient();
  return useMutation({
...
export const useItemGroup = () => {
  ...
  const handleSubmit = async (
    data: ItemGroupDto,
    mode: formType,
    selectedItemGroup?: ItemGroupDto
  ) => {
