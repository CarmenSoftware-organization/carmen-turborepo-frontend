...
import { PaginatedResponseDto } from "@/dtos/paginate.dto";

const API_URL = `${backendApi}/api/config/currencies`;

export const useCurrencyQuery = (token: string, buCode: string, params?: ParamsGetDto) => {
  const { data, isLoading, error } = useQuery<PaginatedResponseDto<CurrencyGetDto>>({
