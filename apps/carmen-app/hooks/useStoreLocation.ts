...
import {
  ...
  createStoreLocation,
  updateStoreLocation,
} from "@/services/store-location.service";
...
export const useStoreLocation = () => {
  ...
  const handleConfirmStatusChange = useCallback(async () => {
    ...
    try {
      ...
      toastSuccess({
        message: `Store location ${updatedStoreLocation.is_active ? "activated" : "deactivated"} successfully`,
      });
    } catch (error) {
      ...
      toastError({
        message:
          error instanceof Error
            ? error.message
            : "Error updating store location status",
      });
    } finally {
      setIsSubmitting(false);
